...
Adventure-detail page (`apps/public/src/routes/adventures/$slug/index.tsx`): rather than the photo/gradient header band (which already carries a photo or the mountain-icon placeholder and shouldn't compete with a second background), the mesh was placed behind the title/badges/pending-banner block through the Phase 29 map-hero section — the actual "hero" content per that phase's own positioning decision, and the segment containing the `glass` trail/spot `Card`s (`TrailsAndSpotsSection`) that benefit most from real color behind them. It stops before the gallery and revision-prose content below, consistent with the design language's exclusion of long-form reading surfaces from glass treatment generally. Verified in-browser (dark mode) on `annapurna-base-camp`: the blobs render behind the glass trail/spot cards in the map-hero sidebar with the green tint clearly bleeding through, text fully legible (task 4's contrast re-check) — no adjustment needed since the glass cards' own background alpha already provides sufficient contrast over the low-opacity blobs, the same headroom validated on the landing page.

**Light mode, closing the gap Phases 30–33 each flagged as unverified**: this environment has no OS-level light/dark toggle exposed to the browser tool, so light/dark mode was forced by walking the live `CSSStyleSheet`/`CSSLayerBlockRule`/`CSSMediaRule` tree via injected JS and rewriting every `@media (prefers-color-scheme: dark)` block's `mediaText` to `not all` (light) or `all` (dark) — 6 such blocks across the two custom glass/body rules in `styles.css` plus Tailwind's own generated `dark:` utility blocks nested inside its `@layer utilities`. A page-local, non-persistent override, not a codebase change.

**Revision after initial review**: the first pass (mesh scoped inside each page's hero/title section, `position: absolute`) technically rendered correctly but read as barely-there in light mode — the blobs sat in corners the actual glass surfaces (sticky header, search bar) rarely overlapped, and the header only ever overlapped the mesh after scrolling past it. Per direct follow-up feedback, `GradientMesh` was changed from an `absolute`, per-page-embedded layer to a single `fixed inset-0 -z-10` layer rendered once in `apps/public/src/routes/__root.tsx` (first child of `<body>`, before the header) — pinned to the viewport rather than the document flow, so the same three blobs stay visible behind every route as the page scrolls past them, not just within one hero section. This also **supersedes this phase's original "landing + adventure-detail only" scope** (task 1/3's page-by-page rollout) — the mesh is now a site-wide background layer, consistent with the "same background everywhere" the fixed-positioning change was asked for. The landing hero's own opaque `bg-gradient-to-b from-primary-50 to-stone-50 dark:from-primary-950/40 dark:to-stone-950` fill was removed in the same pass — it fully hid the newly-global fixed mesh wherever it painted, defeating the point of a single consistent backdrop. Blob opacity was also raised (`primary-400/45`/`accent-400/40`/`primary-300/35` in light, dark values unchanged) and two blobs repositioned to guarantee overlap with the header and search bar without scrolling, rather than relying on their being reachable only deep in a tall hero.
...
**Button fill dropped to fully transparent**: a follow-up asked for the button background itself to be transparent, not just more translucent — `Button.tsx`'s `primary`/`accent`/`secondary`/`danger` variants swapped their `glass-1` fill for `bg-transparent`, keeping the colored border, layered glow, and `backdrop-blur-lg` (blur still distorts what's behind the button even with no tint of its own). The map-toggle button's active state got the same change by hand. Verified in-browser in both light and dark mode: buttons now read as a pure glowing outline + text over whatever sits behind them (the header pill's own glass, in the "Sign in" case), with no fill of their own.

**Glow dropped, borders thickened — the final button treatment**: after several rounds of tuning the neon glow (Phases 38–40), the next two follow-ups removed it entirely and settled the button style as a plain outline: "remove glow effect from all buttons" dropped every `shadow-[...]`/`!shadow-[...]` class (rest, hover, and disabled states) from `primary`/`accent`/`secondary`/`danger`, leaving just the transparent fill, colored border, blur, and a brightened border color on hover — no shadow at all. Then "all buttons must be outline buttons with thicker border" bumped every variant's border from `border`/`border-2` to a uniform `border-[3px]`, and extended the outline treatment to `ghost` for the first time — previously borderless/text-only, it gained its own `border-[3px] border-primary-300/50` (lighter than the other variants, preserving its lower-emphasis role through opacity rather than omitting the border entirely) since the instruction was "all buttons," not just the emphasized ones. The map-toggle button's active state got the same `border-[3px]` bump.

Verified in-browser on a real page with every variant visible at once (`annapurna-base-camp`'s action row — Like/Mark Visited (`secondary`), Tell your story (`accent`), Contribute/History (`ghost`)) in both light and dark mode: clean, glow-free outline buttons with a clearly thicker, saturated border, no shadow bleeding beyond the border edge.
