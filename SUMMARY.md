...
- Background texture pass (Phase 34, built) — a new `GradientMesh` component (three blurred, palette-tinted blob circles, light/dark alpha pairs authored directly rather than as new tokens) gives the glass surfaces from Phases 30–33 real color to refract instead of a flat fill. Landed behind the landing page's existing `TopoLines` hero (user-reviewed and approved before extending further) and, on the adventure-detail page, behind the title/badges through the Phase 29 map-hero section specifically — not the photo header band, and not the gallery/revision-prose content below, keeping it off both a competing photo and long-form reading surfaces. Verified in-browser on the landing page and `annapurna-base-camp` in both dark mode and (via a page-local JS override forcing every `prefers-color-scheme: dark` block off, since this environment has no OS-level toggle) light mode: text and the `glass` trail/spot cards stay fully legible over the blobs in both — closing the light-mode verification gap every prior phase in this round had flagged. This closes out the glassmorphism round. See `GLASSMORPHISM.md` Phase 34 for the full outcome writeup.
