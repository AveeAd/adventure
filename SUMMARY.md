...
## 9. Glassmorphism redesign — glass surface treatment (`apps/public` only, `apps/admin` explicitly out of scope)

**Locked decision**: glassmorphism is a surface *treatment* (translucency, blur, layered depth) applied on top of the existing pine-green/terracotta-on-stone palette, not a palette replacement — and it applies to `apps/public` only, by explicit user decision; `apps/admin`'s AntD `ConfigProvider` theming is untouched by this round.

- Token layer + nav bar showcase (Phase 30, built) — three glass elevation tiers (`--glass-bg-1/2/3`, tinted from the brand palette rather than neutral gray) plus a shared `--glass-border`/`--glass-highlight`/`--glass-shadow` added to `apps/public/src/styles.css`, with light and `prefers-color-scheme: dark` values defined together from the start. Blur radius itself reuses Tailwind's built-in `backdrop-blur-{sm,md,lg}` utilities directly rather than inventing separate blur tokens. Three `.glass-{1,2,3}` classes (background + inset highlight + ambient shadow) are applied to the site header (`apps/public/src/routes/__root.tsx`) — desktop sticky nav and the mobile expanded menu panel both now render as a frosted surface with the page's brand-green tint bleeding through scrolled content, replacing the previous flat `bg-white/90`/`bg-stone-950/90`. Fallback paths landed alongside the tokens, not bolted on later: `@supports not (backdrop-filter)` falls back to a near-solid alpha surface, and `prefers-reduced-transparency`/`prefers-contrast: more` drop translucency and blur entirely in favor of a solid high-contrast surface. Verified in-browser (dark mode, desktop) against the live discover page — sticky header blur confirmed rendering scrolled hero/card content underneath it. See `GLASSMORPHISM.md` Phase 30 for the full token spec and design language.
- Card surfaces (Phase 31, built) — the shared `Card` component (`apps/public/src/components/Card.tsx`) gained an opt-in `glass` boolean prop (glass-2 tier, reusing Phase 30's tokens/fallbacks as-is) rather than restyling every call site, since `Card` also wraps plain forms/panels the design language excludes from glass. Applied to the six actual card-grid surfaces — discover-feed trending/search cards, trip-report and trail/spot cards in the adventure-detail sidebar, and guide-directory cards — while login/edit/account/group/history/comment panels stay solid. Verified in-browser (dark mode) on the guide directory; discover/adventure-detail pages had no seed data to screenshot but share the identical code path. See `GLASSMORPHISM.md` Phase 31 for the full outcome writeup.
- Modals/drawers/popovers/notification dropdown (Phase 32, built) — an audit found no shared `Modal`/`Drawer` component library in `apps/public`; the only two floating chrome surfaces in scope were the notification bell dropdown and the header's account-menu popover (both in `apps/public/src/routes/__root.tsx`/`NotificationBell.tsx`), which moved from a solid `bg-white`/`shadow-lg` panel to the top-tier `glass-3 backdrop-blur-lg` treatment, reusing Phase 30's tokens/fallbacks as-is. The photo lightbox (`fixed inset-0` in the adventure-detail route) was deliberately left solid — a photo-viewer scrim is meant to isolate the image, the opposite intent of letting brand color bleed through. Verified via injected markup against the live dev server rather than a real sign-in (both dropdowns are auth-gated and this environment has no path through Google OAuth) — confirmed blur/tint/legibility render correctly; real sign-in-gated rendering stays unverified, flagged the same way earlier phases flagged their own untested surfaces. See `GLASSMORPHISM.md` Phase 32 for the full outcome writeup.
- Map overlay panels (Phase 33, built) — an audit of every `MapContainer` call site (`AdventureMap.tsx`, `DrawMap.tsx`, `GeodataDiffMap.tsx`) found only one component with floating chrome over the map canvas: `AdventureMap.tsx`'s `FullscreenToggle` and `LocateButton`. There is no separate floating search/filter or elevation-profile panel anywhere in the app — the elevation profile always renders inline in a `Card` below/beside the map, never as a map overlay, so it was out of this phase's scope by definition rather than skipped. Both buttons moved from a solid `bg-white/90`/`dark:bg-stone-900/90` fill to the mid-elevation `glass-2 backdrop-blur-md` treatment, reusing Phase 30's tokens/fallbacks as-is; their prior `hover:bg-white` states (which the unlayered `.glass-2` background rule would have silently overridden anyway, per Phase 30's cascade-priority note) were replaced with `hover:opacity-80` so the hover affordance is actually visible. Verified in-browser (dark mode) on both the discover-feed map and the Annapurna Base Camp adventure-detail page's `MapHeroLayout` split-pane view — both buttons render a legible translucent surface directly over live map tiles with no stacked-blur jank. See `GLASSMORPHISM.md` Phase 33 for the full outcome writeup.
- Background texture pass (Phase 34, built) — a new `GradientMesh` component (three blurred, palette-tinted blob circles plus the former `TopoLines` contour-line SVG, light/dark alpha pairs authored directly rather than as new tokens) gives the glass surfaces from Phases 30–33 real color and texture to refract instead of a flat fill. Initially landed per-page (landing hero + adventure-detail map-hero section) with `position: absolute`, but a follow-up review found the blobs barely readable in light mode and invisible once scrolled past — revised to a single `fixed inset-0 -z-10` layer rendered once in `__root.tsx`, pinned to the viewport so the same background shows through every glass surface on every route regardless of scroll position, superseding this phase's original two-page scope. The landing hero's own opaque gradient fill was removed (it fully hid the new global layer), blob opacity was raised, two blobs repositioned to guarantee overlap with the header/search bar, and `TopoLines` (previously its own component, hero-only) was folded into the same layer with its fade mask changed from top-anchored to a symmetric center-out fade so it reads as vertically centered in the viewport rather than clipped to the top. Verified in-browser across light and dark mode (via a page-local JS override forcing `prefers-color-scheme` since this environment has no OS-level toggle) on the landing page, `annapurna-base-camp`, and `/guides` (a route that never had this treatment before, confirming it's genuinely global) — text and glass cards stay fully legible, and the background holds steady while scrolling. See `GLASSMORPHISM.md` Phase 34 for the full outcome writeup.
- Floating header (Phase 35, built) — direct follow-up once the header sat above the new site-wide mesh: the header's glass background/border/blur moved off the full-width `<header>` strip and, per a further follow-up, split into two independent rounded pills sitting at opposite ends of a `flex justify-between` shell (`sticky top-0 px-4 pt-4`) — a left pill holding just the logo/site name, and a right pill (`hidden sm:flex`, replaced by its own small circular pill for the hamburger toggle on mobile) holding the nav links, divider, and `AuthStatus` — rather than one continuous bar, so the mesh shows through the gap between them too. Active-link styling was deliberately left as its original waypoint-dot indicator rather than adopting the filled-pill look from the layout reference image, per explicit feedback that only the positioning was being requested. Both pills' padding was bumped up a size in the same session per follow-up feedback. The mobile menu panel kept its own separate floating bar below the row. Verified in-browser at both scroll-top and scrolled desktop states — sticky positioning, blur, and legibility all unchanged from Phase 30, just split across two floating surfaces instead of one. This closes out the glassmorphism round. See `GLASSMORPHISM.md` Phase 35 for the full outcome writeup.

- Landing search bar (Phase 36, built) — the landing-page hero's search `<input>` already had an ad hoc translucent/blurred look predating this round; swapped its bespoke `bg-white/60`/`border-stone-300/70` fill and border for the shared `glass-2`/`--glass-border` tokens used everywhere else, and dropped its now-dead `shadow-sm`/`hover:shadow-md`/`focus:shadow-md` utilities (`.glass-2`'s plain-CSS box-shadow already wins that cascade per Phase 30's note, so they painted nothing). Verified in-browser in both light and dark mode. See `GLASSMORPHISM.md` Phase 36 for the full outcome writeup.

- Stronger glass tuning (Phase 37, built) — per direct feedback that the glass tiers read as flat translucent panels rather than real glass, all three `--glass-bg-*` alpha values were lowered ~25–30% (light and dark) and every `backdrop-blur-*` utility on a `.glass-*` surface bumped one Tailwind step (`md`→`lg`, `lg`→`xl`) site-wide — header pills, cards, map buttons, the landing search bar, and both dropdowns. Fallback (`--glass-fallback-bg-*`) values were left untouched since they exist specifically to not be translucent. Also raised `--glass-shadow`'s spread and alpha in the same pass (`0 8px 32px`→`0 16px 48px`, light alpha 0.12→0.2, dark 0.45→0.55) so surfaces read as more lifted/elevated, not just more transparent. Verified in-browser in both light and dark mode: scrolled content is now clearly legible-but-blurred through the header pills, with a visibly stronger drop shadow underneath — a stronger frosted-glass, more-elevated read than before. See `GLASSMORPHISM.md` Phase 37 for the full outcome writeup.

**Phased build plan**: see `GLASSMORPHISM.md` — Phase 30 (tokens + nav bar, **built**) → Phase 31 (card surfaces, **built**) → Phase 32 (modals/drawers/popovers/notification dropdown, **built**) → Phase 33 (map overlay panels, **built**) → Phase 34 (background gradient/texture pass, **built**) → Phase 35 (floating header, **built**) → Phase 36 (landing search bar, **built**) → Phase 37 (stronger glass tuning, **built**). This closes out the glassmorphism round; that doc is the phase-by-phase spec including the performance budget (stacked-blur cap) and accessibility checklist this round follows, this section stays the summary/outcome record.
