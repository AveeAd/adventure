...
- **Guardrails**:
  ...
  - Reject files with no `<trkpt>` elements — **and, since some tools (route planners, Movescount's route export) emit `<rte>`/`<rtept>` instead of `<trk>`/`<trkseg>`/`<trkpt>`, fall back to reading a route's points when the file has no tracks at all**, rather than rejecting a file that has real path data just because it isn't shaped as a track. Fixed post-launch after a real upload hit this (no per-point timestamps or segments in a route, but the same ordered lat/lng(/ele) shape).
