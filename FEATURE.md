...
| Phase | Name | Status | Depends on (this milestone) |
|---|---|---|---|
| 15 | Geodata changeset history — GEODATA_HISTORY.md (`TrailRevision`/`SpotRevision`, revision-scoped confirmations, supersedes FEATURE.md §4's `deleteMany`-on-edit rule) | **done** | none |
| 16 | Trail elevation profiles + activity tracks — TRAIL_ELEVATION.md (GPX import, `TrailElevationProfile`) and ACTIVITY_TRACKS.md (`ActivityTrack`, KML/KMZ/GeoJSON import, promote-to-trail) built together, one parser module | **done**, with 3 pieces named-not-built (offline-bundle endpoint, `ST_DWithin`, propose-trail-update UI — see ACTIVITY_TRACKS.md's status line) | Phase 15 (so promotion/invalidation target the revision model directly) |
| 17 | Spatially-derived district tagging — FEATURE.md §4 (`District.boundary` import, `ST_Intersects`/`ST_Contains` derivation, `AdventurePageDistrict.source`) | **done** | none, but sequenced after Phase 16 to keep its own trails/spots-adjacent migration clean of the other two — see the known spurious-`DROP INDEX` gotcha in §4's Migration notes |
| 18 | UI i18n — I18N.md (wire the library, extract every string to a catalogue, ship `en` only) | **infra done, string extraction partial** — see I18N.md's "What's actually extracted" | none — fully orthogonal to the geodata work above, sequenced last because it's independent and lower urgency, and benefits from a stable `Notification.message` string set once Phases 15–17 stop touching notification-triggering copy |
...
- ~~**No revision history, unlike `PageRevision`.** Editing updates the row in place; history is "created by" + "last edited by" only — a deliberate simplification. This creates a real gap: `PageConfirmation` ties to a revision precisely so an edit can't ride on stale trust, and geodata has no revision to tie to. The mitigation is a **service-layer rule, not a schema constraint**: any edit to geometry or key fields resets `verificationStatus` and deletes existing confirmations in the same transaction.~~ **Superseded — GEODATA_HISTORY.md shipped (Milestone 2 Phase 15).** `TrailRevision`/`SpotRevision` close this gap and confirmations are retargeted to the revision; the `deleteMany`-on-edit rule described here is retired. Kept above for history, not as the current rule — see GEODATA_HISTORY.md and its per-table notes for what replaced it.
...
### Spatially-derived district tagging — schema (built, Milestone 2 Phase 17)
...
`AdventurePageDistrict` previously had **no timestamps at all** — a standing violation of §2's "`createdAt`/`updatedAt` on every table" convention, fixed in the same migration (`20260801000000_district_tagging`).

**Boundary import**: a one-off script `apps/api/prisma/scripts/import-district-boundaries.ts` (also wired into `npm run seed:all`), fixture `apps/api/prisma/seed-data/nepal-district-boundaries.geojson`, matched to existing districts **by slug**, idempotent upsert — the same pattern `import-locations.ts` already establishes. **As built**: sourced from the `nepal-geojson` npm package (not OSM/HDX directly) — that package is not an app dependency, it was installed in a throwaway directory purely to extract its 77 per-district municipality-level polygon files, unioned per district (`ST_Union`) and simplified (`ST_SimplifyPreserveTopology`, ~0.003° tolerance) down to one `MultiPolygon` each, producing a ~410KB fixture (well under the "tens of MB" full-resolution warning). Nine district-name aliases needed hand-mapping to this project's canonical slugs (e.g. `Chitawan`→`chitwan`, `Nawalparasi_E`→`parasi`, `Rukum_W`→`rukum-west`) — all 77 districts matched after that.

**Derivation rule, as built**: `deriveDistrictTags()` (`apps/api/src/geodata/district-derivation.util.ts`) fires inside the same transaction as `TrailsService.create`/`update`/`importGpx` and `SpotsService.create`/`update` (only on updates that actually change geometry) — never client-driven. `ST_Intersects(d.boundary, t.geometry)` for trails, `ST_Contains(d.boundary, s.geometry)` for spots, `INSERT ... ON CONFLICT ("adventurePageId", "districtId") DO NOTHING`. Verified: creating a trail crossing two districts tags both `DERIVED` in one transaction; a name-only edit (no geometry change) derives nothing.

**The required fix — applied**: `AdventurePagesService.updateMetadata`'s district delete is narrowed to `{ adventurePageId: id, source: 'MANUAL' }`, and district creation is now an upsert (not `createMany`) so a contributor manually picking a district that already has a `DERIVED` row upgrades it to `MANUAL` in place, rather than hitting the unique-constraint collision `createMany` would. Verified end-to-end: a `DERIVED` `kaski` tag survived a metadata edit that manually picked `myagdi` (which upgraded from `DERIVED` to `MANUAL`), matching "MANUAL always wins, derivation is additive-only."

**Migration ordering**: `District.boundary` added the third hand-added GiST index (`districts_boundary_idx`, `CREATE INDEX IF NOT EXISTS`) — the migration touches only `districts`/`adventure_page_districts`, no trails/spots DDL, so it carries none of the known spurious-`DROP INDEX` risk.

**Open decisions, as left**: derivation was **not backfilled** across existing pages — it only fires going forward from a create/geometry-update, per "whether to backfill... or only derive going forward" being left open; `Municipality` boundaries were **not added** (district-level only, matching the "finer tagging, much bigger fixture" tradeoff named here); no intersection-length threshold was built for a border-hugging route picking up a neighbouring district — a small clip derives a tag same as a large one, left as designed-open.
...
**Superseded by GEODATA_HISTORY.md (Milestone 2 Phase 15, shipped)** — kept here for history; see that doc's Service-layer notes for the rule as it actually ships today.

- ~~**Create**: `Trail`/`Spot` insert always requires `adventurePageId`.~~ Still true, now also creates a `version: 1` `TrailRevision`/`SpotRevision` in the same transaction.
- ~~**Edit**: any update to geometry or key fields resets `verificationStatus` (to `UNVERIFIED`, or `NEEDS_REVIEW` if the submitter flags the edit as safety-critical — a transient request flag, not a stored column, since there's no permanent per-edit record to attach it to) **and** deletes the row's existing confirmations, in the same transaction.~~ The `verificationStatus` reset stays (and `isSafetyCriticalEdit` is now a stored column on the new revision, not a transient flag); the confirmation-delete is retired — confirmations go stale for free by pointing at a superseded revision.
- ~~**Confirm**: upsert a `TrailConfirmation`/`SpotConfirmation` for `(row, current user)`; crossing a threshold flips `verificationStatus` to `VERIFIED`.~~ Now upserts for `(current revision, current user)` and counts against that revision, not the row's all-time total.
