...
| 18 | UI i18n — I18N.md (wire the library, extract every string to a catalogue, ship `en` only) | not started | none — fully orthogonal to the geodata work above, sequenced last because it's independent and lower urgency, and benefits from a stable `Notification.message` string set once Phases 15–17 stop touching notification-triggering copy |
