...
| **23 — Reporting & enforcement** | `ContentReport`, filing UI, resolution flow, revert-on-uphold reusing the Phase 15 revert path, penalties, rate limiting, `Media.isActive` | Revert-on-uphold turned out not to be a literal call to Phase 15's `revert()` (that just files another PENDING revision) - each service instead grew a `revertToPreviousApproved()` that applies the previous approved snapshot directly and forces `verificationStatus = NEEDS_REVIEW` (a new `deriveVerificationStatus` param). Also pulled two `NotificationType` values (`REPORT_RESOLVED`, `REPORT_UPHELD_AGAINST_YOU`) forward from Phase 25, since "editor and reporter both notified" is stated Phase 23 scope |
| **24 — Moderation console** | `ModeratorApplication` + application UI + admin review, admin settings resource, moderator-restricted resource set | Pulled `MODERATOR_APPLICATION_DECIDED` forward from Phase 25's notification sweep, same precedent Phase 23 set for the two `REPORT_*` types - §7's "and notifies" is this phase's requirement, not the later sweep's. The "moderator-restricted resource set" turned out to be almost entirely already true server-side (base CRUD's write routes, `users.controller.ts`'s PATCH, and guide-profiles' `verification-status` were already `@Roles(Role.ADMIN)`-only since Phase 19); what Phase 24 actually added was the UI-side mirror - a Refine `accessControlProvider` (hides Create/Edit/Delete buttons and the two admin-only resources from the Sider) plus a `RestrictedRoute` guard on the same routes for direct-URL access, so a moderator gets a "not authorized" page or a hidden button instead of a form that 403s on submit. No public read endpoint exists for `SystemSetting` (admin-only per §2.1), so the Account page's moderator-application gate hardcodes the documented default (level 25) the same way the review queue already hardcodes level 10, rather than inventing one. |
